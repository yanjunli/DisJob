...
import com.hqyg.disjob.rpc.client.handler.EjobKillResponseHandler;
import com.hqyg.disjob.rpc.client.handler.EjobRestartResponseHandler;
...
import com.hqyg.disjob.rpc.codec.EjobTaskDecoder;
import com.hqyg.disjob.rpc.codec.Header;
import com.hqyg.disjob.rpc.codec.RpcEncoder;
...
import com.hqyg.disjob.rpc.utils.RpcConstants;
...
public class ChannelBootStrap {
	...
	public ChannelBootStrap(final PhpTaskCmd cmd, final EjobResponse response,
		...
		bootstrap.handler(new ChannelInitializer<SocketChannel>() {
			...
			public void initChannel(SocketChannel channel) throws Exception {
				...
				pipeline.addLast(new EjobTaskDecoder(cmd,
						RpcConstants.MAX_FRAME_LENGTH,
						RpcConstants.LENGTH_FIELD_OFFSET,
						RpcConstants.LENGTH_FIELD_LENGTH));
				pipeline.addLast(new RpcEncoder(RpcRequest.class));
				EjobResponse tmp = response;
				if (cmd == cmd.RESTART) {
					pipeline.addLast(new EjobRestartResponseHandler(cmd,(EjobRestartTaskResponse) tmp, downLatch));
				} else if (cmd == cmd.KILLTASK) {
					pipeline.addLast(new EjobKillResponseHandler(cmd,(EjobKillTaskResponse) tmp, downLatch));
				}
