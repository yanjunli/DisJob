package com.hqyg.disjob.console.service.impl;
...
import com.hqyg.disjob.monitor.db.mappers.DBJobBasicInfoMapper;
...
import com.hqyg.disjob.rpc.client.proxy.ChannelBootStrap;
import com.hqyg.disjob.rpc.codec.EjobKillTaskResponse;
import com.hqyg.disjob.rpc.codec.EjobResponse;
import com.hqyg.disjob.rpc.codec.EjobRestartTaskResponse;
import com.hqyg.disjob.rpc.codec.Header;
import com.hqyg.disjob.rpc.codec.RpcRequest;
import com.hqyg.disjob.rpc.codec.RpcRequestData;
import com.hqyg.disjob.rpc.utils.PhpTaskCmd;
...
public class JobInfoServiceImpl implements JobInfoService {
		...
		if(urlList !=null && urlList.size() >0){
			for(HURL url : urlList){
				...
				int port = url.getPort();
				 int code =  requestCmd(requestId, host, port, cmd,data,new EjobKillTaskResponse());
	...
	private List<String> process(String groupName,String jobName, PhpTaskCmd cmd,RpcRequest data) {
		...
		if(urlList !=null && urlList.size() >0){
			for(HURL url : urlList){
				String host = url.getHost();
				int port = url.getPort();
				 int code =  requestCmd(null, host, port,cmd,data,new EjobRestartTaskResponse());
				 result.add(host+"_"+port+":"+code);
 			}
 		}else{
	...
	private int requestCmd(String requestId, String host, int port,final PhpTaskCmd cmd,RpcRequest data,EjobResponse response) {
		//EjobKillTaskResponse response = new EjobKillTaskResponse();
		...
		if(response !=null){
			//状态统一转化为code处理
			if(response instanceof EjobRestartTaskResponse){
				return ((EjobRestartTaskResponse)response ).isStatus() ? 1 :0;
 			}else if(response instanceof EjobKillTaskResponse){
				return ((EjobKillTaskResponse)response ).getCode();
 			}else{
 				return -1;
 			}
		}else{
	...
	public List<String> restartJob(String groupName,String jobName) {
		...
		return result;
		/*EjobRestartTaskResponse response = new EjobRestartTaskResponse();
		CountDownLatch downLatch = new CountDownLatch(1);
