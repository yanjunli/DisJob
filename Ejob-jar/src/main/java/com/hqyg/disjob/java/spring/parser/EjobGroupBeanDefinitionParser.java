...
import com.hqyg.disjob.java.spring.bean.EjobGroup;
 ...
 *
 */
public class EjobGroupBeanDefinitionParser extends AbstractSingleBeanDefinitionParser {
	public static Set<String> sers = new HashSet<String>();
	protected Class<EjobGroup> getBeanClass(Element element) {
		return EjobGroup.class;
	}
	...
	protected void doParse(Element element, BeanDefinitionBuilder bean) {
		...
		for (int i = 0; i < elementSons.getLength(); i++) {
			...
			JobInfo ejob = new JobInfo(classname,groupName,jobName,cron,fireNow);
			list.add(ejob);
		}
