package com.hqyg.disjob.java.listener;

import com.hqyg.disjob.event.ObjectEvent;
import com.hqyg.disjob.event.ObjectListener;
import com.hqyg.disjob.java.ProviderClassName;
import com.hqyg.disjob.java.bean.StartUpConfig;
import com.hqyg.disjob.java.core.dispatcher.EventObjectDispatcher;

public class EjobStartUpListener implements ProviderClassName,ObjectListener<StartUpConfig> {
	
	public void onEvent(ObjectEvent<StartUpConfig> event) {
		//这里启动有个先后顺序：初始化参数配置->启动netty 监听 --> 初始化 curator
		System.out.println("ejob initinalize finish.");
		EventObjectDispatcher.dispatcherEjobStartUpFinish();
	}
	public String getClassName() {
		return this.getClassName();
	}
	
}
