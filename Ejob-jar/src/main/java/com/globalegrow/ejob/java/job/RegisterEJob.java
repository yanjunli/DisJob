...
public class RegisterEJob implements ProviderClassName{
	private JobInfo jobInfo;
	public RegisterEJob(JobInfo jobInfo) {
	...
	private void createJob(JobInfo jobInfo) throws Exception{
		...
		//1、添加一个新的Job
		String tmpPrefix = EjobConfigService.getClusterName().length() <=0 ? "" : EjobConfigService.getClusterName()+"_";
		...
		int port = EjobConfigService.getServerPort();
		//2、创建 /ejob-dev/rpc/global/job/providers/ip:9501 这个数据节点，并写入需要调度的基本信息
		String ipNode = path +"/providers/"+ip+":"+port;
		if(null == client.checkExists().forPath(ipNode)){
			//3、ejob://10.40.6.89:9501/test?serverGroup=oms&phpFilePath=/usr/local/php-test/TestService.php&className=TestService&methodName=test&version=1
			StringBuffer sb = new StringBuffer();
			sb.append("ejob://");
			sb.append(ip+":"+EjobConfigService.getServerPort()+"/");
