package com.hqyg.disjob.java.service;
...
import com.hqyg.disjob.java.CronExpression;
import com.hqyg.disjob.java.EjobConstants;
import com.hqyg.disjob.java.ExecutorBuilder;
import com.hqyg.disjob.java.action.SendBeginExeTimeAction;
import com.hqyg.disjob.java.action.SendCompleteTimeAction;
...
import com.hqyg.disjob.java.job.RegisterEJob;
...
public class JobService{
	...
	private static ConcurrentHashMap<String, Class<? extends EJob>> DYNAMIC_JOBS = new ConcurrentHashMap<String, Class<? extends EJob>>();
	
	private static ConcurrentHashMap<String, EJob> EJOB_OBJECT_MAP = new ConcurrentHashMap<String, EJob>();
	...
	public static void setEjobClass(String className,Class<? extends EJob> ejobClass){
		
		DYNAMIC_JOBS.putIfAbsent(className, ejobClass);
	}
	
	public static Class<? extends EJob> getEjobClass(String className){
		
		return DYNAMIC_JOBS.get(className);
	}
	
	public static void setEjobInstance(String className,EJob ejob){
		
		EJOB_OBJECT_MAP.putIfAbsent(className, ejob);
	}
	
	public static EJob getEjobInstance(String className){
		
		return EJOB_OBJECT_MAP.get(className);
	}
	/**
	 * 
	 * @param className
	...
	public static boolean doInitJob(List<JobInfo> jobInfos){
		for(JobInfo jobInfo:jobInfos){
			...
			groupQueue.get().enqueue(new RegisterEJobAction(new RegisterEJob(jobInfo),groupQueue.get()));
		}
		return true ;
	}
	...
	private static JobInfo getJobInfo(String className){
		Class<? extends EJob> clazz = JobService.getEjobClass(className) ;
		if(clazz == null){
			try {
				clazz = (Class<? extends EJob>) Class.forName(className);
			} catch (ClassNotFoundException e) {
				e.printStackTrace();
				Log.error("get job info exception:", e);
				throw new RuntimeException(e);
			}
			short startUpType = EjobConfigService.getStartupType() ;
			/**
			 * 因为如果不是spring 启动，那么job 的实例是通过反射得到的。故这里预先缓存这个类的class
			 */
			if(startUpType == EjobConstants.StartUpType.JAVA_APPLICATION||startUpType == EjobConstants.StartUpType.WEB_SERVLET_START_UP){
				JobService.setEjobClass(className, clazz);
			}
		}
	...
	public static void handlerExecuter(RpcContainer rpcContainer,EJob action,Channel channel){
		if(action == null){
			return ;
		}
	...
	@SuppressWarnings("unchecked")
	public static EJob getEJobAction(String className, String methodName){
		Object job = null ;
		EJob action = null;
		if(EjobConfigService.getStartupType() == EjobConstants.StartUpType.WEB_SERVLET_START_UP || EjobConfigService.getStartupType() == EjobConstants.StartUpType.JAVA_APPLICATION){
			action = getEjobInstance(className);
			if(action == null){
				/**
				...
				try {
					clazz = (Class<? extends EJob>) (clazz == null ? Class.forName(className):clazz);
				...
				if(EJob.class.isInstance(job) && "execute".equals(methodName)){
					action = (EJob) job;
					JobService.setEjobInstance(className, action);
				}
			}
		}
		return action;
