...
import com.hqyg.disjob.register.repository.election.LeaderElectionApi;
import com.hqyg.disjob.register.repository.election.LeaderElectionApiImpl;
import com.hqyg.disjob.register.repository.watch.WatchApi;
import com.hqyg.disjob.register.repository.watch.listener.ConnectionStateListenerImpl;
import com.hqyg.disjob.register.repository.watch.listener.SchedulerJobInitListener;
import com.hqyg.disjob.register.repository.watch.listener.SessionNodeListener;
...
/**
 * Ejob Server注册类： 1、初始化ejob、rpc、scheduler三大节点 2、job初始化 3、job分配初始化
 * 4、job监听及动态分配到slave机器 5、slave宕机，进行slave所属机的任务分配 6、ejob动态扩容
 * 
 * @author Disjob
 * @date 创建时间：2016-5-19
 */
...
public class ServerZKRegistry extends AbstractZKRegistryCenter {
	...
	public void init() {
		...
		try {
			...
			selectLeader();
			com.hqyg.disjob.slaver.utils.SlaveUtils.leaderLatch = leaderLatch;
			...
			watchApi.pathChildrenWatch(client, Constants.EJOB_SERVER_NODE_SESSION, false, new SessionNodeListener());
	...
	protected boolean initRootNode() {
		...
		if(!znodeApi.checkExists(client, slavePathHostName)){
 		...
 		if(!znodeApi.checkExists(client, Constants.EJOB_SERVER_NODE_SESSION)){
 			znodeApi.createPersistent(client, Constants.EJOB_SERVER_NODE_SESSION,null);
		}
 		if(!znodeApi.checkExists(client, Constants.EJOB_SERVER_NODE_PROJECT)){
 			znodeApi.createPersistent(client, Constants.EJOB_SERVER_NODE_PROJECT,null);
		}
	...
	private void selectLeader() {
		String masterPath = znodeApi.makePath(Constants.ROOT, Constants.EJOB_SERVER_NODE_ROOT,
				Constants.EJOB_SERVER_NODE_MASTER);
			...
			leaderLatch.addListener(new LeaderLatchListener() {
				...
				public void isLeader() {
 				//	setMasterIp();
					...
					// 成为leader的时候执行
					LoggerUtil.info("Ejob:HostName:" + localHost.getHostName() + " IP:" + localHost.getIp() + "成功成为leader");
	...
	/**
	 * 设置/ejob/scheduler/master/host->masterIp
	 */
	private void setMasterIp() {

		String masterIp = znodeApi.getData(client, Constants.EJOB_SERVER_NODE_MASTER_IP);
 		if(StringUtils.isNotEmpty(masterIp)){
			znodeApi.update(client, Constants.EJOB_SERVER_NODE_MASTER_IP, localHost.getIp());      
		}
		 else{
			 //znodeApi.createPersistent(client, Constants.EJOB_SERVER_NODE_MASTER_IP, localHost.getIp()); 
			 znodeApi.createPersistent(client, Constants.EJOB_SERVER_NODE_MASTER_IP, localHost.getIp());
        }
			...
			if (StringUtils.isNoneBlank(jobNameStr) && jobNameStr.length() > 0) {
				...
				for (String jobName : jobNameArray) {
					...
					if (job != null) {
						...
						LoggerUtil.warn("Can not find job, groupName:" + groupName + " jobName:" + jobName + " on /ejob/job node");
					}
				}
			}
		}
	...
	public void  buildSessionBindCache(){
		com.hqyg.disjob.slaver.utils.SlaveUtils.buildSessionHostMap(client);
		com.hqyg.disjob.slaver.utils.SlaveUtils.buildProjectSessionMap(client);
	}
	...
	public static String rpcRootNode = ZKPaths.makePath(Constants.ROOT, Constants.EJOB_RPC_NODE_ROOT);
	public static String masterPath = ZKPaths.makePath(Constants.ROOT, Constants.EJOB_SERVER_NODE_ROOT, Constants.EJOB_SERVER_NODE_MASTER);
	public static String slavePath = ZKPaths.makePath(Constants.ROOT, Constants.EJOB_SERVER_NODE_ROOT, Constants.EJOB_SERVER_NODE_SLAVE);
	public static String slaveServerPath = ZKPaths.makePath(slavePath, Constants.PATH_SEPARATOR + localHost.getIp());
	public static String slavePathStatus = ZKPaths.makePath(slaveServerPath, Constants.EJOB_SERVER_NODE_SLAVE_STATUS);
	public static String slavePathHostName = ZKPaths.makePath(slaveServerPath, Constants.EJOB_SERVER_NODE_SLAVE_HOSTNAME);
	public static String slavePathExecution = ZKPaths.makePath(slaveServerPath, Constants.EJOB_SERVER_NODE_SLAVE_EXECUTION);
}
