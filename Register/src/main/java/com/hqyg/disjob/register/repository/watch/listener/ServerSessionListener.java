 ...
 * 
 *  Description:
 *  监听ejob/session/会话名称/ip:port会话节点
 *  功能：监听ejob/session/会话名称下临时节点的增加和移除事件
 *  2.监听到临时会话节点变动则动态更改/ejob/rpc/group/jobname/providers/ip:port节点的值并维护一份缓存
 *  Revision History
 *
 *  Date：		2016年11月15日
 *  Author：		Disjob
...
public class ServerSessionListener implements PathChildrenCacheListener{
	...
	public void childEvent(CuratorFramework client, PathChildrenCacheEvent event)throws Exception {
 		 ...
 		 if(event.getType() == org.apache.curator.framework.recipes.cache.PathChildrenCacheEvent.Type.CHILD_ADDED){
 	         String path = event.getData().getPath();//->path:/ejob/session/会话名称/ip:port
 	        ...
 	        if (isMaster) {
 	        	try {
 	        		String groupNode = znode.makePath(Constants.EJOB_SERVER_NODE_PROJECT,Constants.PATH_SEPARATOR+sessionName);
		 ...
		 if(event.getType() == org.apache.curator.framework.recipes.cache.PathChildrenCacheEvent.Type.CHILD_REMOVED){
			//->path:/ejob/session/会话名称/ip:port
