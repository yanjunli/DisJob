...
import com.hqyg.disjob.common.util.Bean2MapAndMap2Bean;
...
import com.hqyg.disjob.register.core.jobs.JobFireFactory;
...
public class JobOperationServiceImpl implements JobOperationService {
	...
	@Resource
	public   EjobServerService initServerExecuteJobService;
	...
	public static final String pathFormat = ZKPaths.makePath(Constants.ROOT, Constants.EJOB_RPC_NODE_ROOT, "%s", "%s", Constants.EJOB_PROVIDERS, "%s"); 
	...
	/**
	 *  String data = "ejob://10.40.6.100:9501/%s?serverGroup=%s&phpFilePath=/usr/local/php-test/TestService.php&className=TestService&methodName=test1&version=1";
	 *  String path = "/ejob-dev/rpc/%s/%s/providers/10.40.6.100:9501";
	 */
	@Override
	public boolean addJob(Job job, String username) throws ZKNodeException2 {
		String jobName = job.getJobName();
		String groupName = job.getGroupName();
		String rpcJobPath = znodeApi.makePath(Constants.ROOT, Constants.EJOB_RPC_NODE_ROOT,
		...
		if(znodeApi.checkExists(client, rpcJobPath)){
			throw new EjobFrameWorkException("任务已经存在, 任务组[" + groupName + "], 任务[" + jobName + "]");
		}
		...
		if (!hosts.isEmpty()) {
			...
			throw new EjobFrameWorkException(String.format("任务组 [%s] 绑定的会话中找不到可用的地址", groupName));
		}
	}
	...
	private void checkJobBeforeUpdateAndSave(Job job) {
		...
		if (boardcast) {
			...
			if (StringUtils.isNotEmpty(parameters) && parameters.contains(",")) {
				throw new EjobFrameWorkException("广播模式的任务不支持分片式的任务参数,参数中不可以包含[,]符号");
			}
		}
	}
	...
	private List<String> getHostByGroupName(CuratorFramework client, String groupName) {
		...
		if(sessions != null){
			...
			if(hostList.isEmpty()){
				throw new EjobFrameWorkException(String.format("任务组  [%s] 绑定的会话找不到url", groupName));
			}else{
				return hostList;
			}
		}else{
			throw new EjobFrameWorkException(String.format("任务组  [%s] 没有绑定任何会话", groupName));
		}
	}
	...
	public boolean suspendJob(Job job) {
		...
		try {
			...
			if (StringUtils.isNoneBlank(data)) {
				...
				/**
				 * 1、先更改/ejob/job/config 下面的信息，然后通知slave节点重新装载job.这个时候 被暂停的job 不会装到quartz 里面去
				 * 2、得到 /slave/job上某group上移除该job值后得到的job值
				 * 3、如果group下的job的值为空串->单个job被移除,则直接remove掉该job节点
				 */
				stopJob.setJobStatus(3);
				stopJob.setSlaveIp(""); // 更改ejob/job的对应job节点的值
	      ...
	      try{
             ...
             if(StringUtils.isNotEmpty(data)){
               // Job stopJob = new Job();
   	         ...
   	         //更改激活状态的时候发布到/ejob/scheduler/slave上,不允许从激活到未激活,所以只要是status=0的就直接进入激活流程
 	        //恢复状态->给job分配一个slave
             LoggerUtil.info("job resume, job is "+job.toString());
             SlaveUtils.refreshSlaveIp(client);
             String slaveIp = com.hqyg.disjob.slaver.utils.SlaveUtils.distributeSlave(client, group, jobName,transaction);
             //设置分配后ip
             ...
             com.hqyg.disjob.slaver.utils.SlaveUtils.updateSlaveExecution(client, group, jobName, transactionFinal, resumeJob.getSlaveIp());
//             znodeApi.commitTransaction(transactionFinal);
	...
	public boolean createGroup(JobGroup jobGroup, String username){
		...
		String newGroupPath = ZKPaths.makePath(Constants.ROOT, Constants.EJOB_RPC_NODE_ROOT, groupName);
		if(znode.checkExists(client, newGroupPath)){
			throw new EjobFrameWorkException("任务组已经存在 " + groupName);
		}else{
			...
			try {
				...
				throw new EjobFrameWorkException("任务创建失败 ," + e.getMessage());
			}
			return true;
		}
	}
    ...
    public List<String> getAllGroup()
    {
        CuratorFramework client = threadLocalClient.getCuratorClient().getCuratorClient();
        ZnodeApiCuratorImpl znode = new ZnodeApiCuratorImpl();
        List<String> currentChilds = znode.getChildren(client, Constants.ROOT+Constants.EJOB_RPC_NODE_ROOT);
    ...
    public List<Job> getJobListByGroup(String groupName)
    {
       ...
       List<String> currentChilds = znode.getChildren(client, Constants.ROOT+Constants.EJOB_RPC_NODE_ROOT+Constants.PATH_SEPARATOR+groupName);
				...
				if (tmJob.equals(jobName)) {
					...
					if (StringUtils.isNotEmpty(data)) {
						...
						if (StringUtils.isEmpty(job.getFilePath())) {
							String node = znodeApi.makePath(Constants.ROOT, Constants.EJOB_RPC_NODE_ROOT, groupName);
							String rpcNodePath = znodeApi.makePath(node, jobName, Constants.EJOB_PROVIDERS);
							if (znodeApi.checkExists(client, rpcNodePath)) {
								if (znodeApi.getChildren(client, rpcNodePath).size() != 0) {
									...
									String ejobRPCHostdata = znodeApi.getData(client, hostPath);

									fillJobData(ejobRPCHostdata, job);
								}
							}
						}
						result = job;
...
//    		job.setSlaveIp(slaveIp);
			...
			throw new EjobFrameWorkException("识别到该job缺少filePath, 从rpc/provider解析相关数据时异常 :" + e);
		}
    }
    ...
    /**
     * 1.清除/ejob/schedule/slave/ip下清除所有job
     * 2.根据/ejob/job/group/job/config分配job
     * 
     */
	@Override
    public int averageDistributeSlaveJob() throws Exception {
        ...
        CuratorFramework client = threadLocalClient.getCuratorClient().getCuratorClient();
        ConcurrentHashSet<EjobServerInfo> availableIps =  com.hqyg.disjob.slaver.utils.SlaveUtils.refreshSlaveIp(client);
        if(availableIps.size() < 2 ){ //集群中可用机器少于2台,拒绝job分配
        	return com.hqyg.disjob.register.job.JobOperatorStatus.WARN.getValue();
        }
		...
		return com.hqyg.disjob.register.job.JobOperatorStatus.SUCCESS.getValue();
 	}
	...
	public boolean updateJob(CuratorFramework client, Job job) {
		try {
			...
			/**
			 * 1、更改激活状态的时候发布到/ejob/scheduler/slave上,不允许从激活到未激活,所以只要是status=0
			...
			synchronized (this.getClass()) {
				if(reloadToQuartz){
					com.hqyg.disjob.slaver.utils.SlaveUtils.updateSlaveExecution(client, group, jobName, transactionFinal, job.getSlaveIp());
				}
				...
				if(reloadToQuartz || checkUpdateQuartz(job,zkJob)) {
					String groupNode = nodeApi.makePath(Constants.ROOT,Constants.EJOB_SERVER_NODE_ROOT,Constants.EJOB_SERVER_NODE_SLAVE,
							Constants.PATH_SEPARATOR + zkJob.getSlaveIp(),Constants.EJOB_SERVER_NODE_SLAVE_EXECUTION,Constants.PATH_SEPARATOR + group);
...
//			znodeApi.commitTransaction(transactionFinal);				
	...
	public boolean saveJob(Job job) {
		   ...
		   String rpcPath = znodeApi.makePath(Constants.ROOT, Constants.EJOB_RPC_NODE_ROOT,Constants.PATH_SEPARATOR+job.getGroupName(),Constants.PATH_SEPARATOR+job.getJobName(), Constants.PATH_SEPARATOR+Constants.EJOB_PROVIDERS);
	...
	public boolean bindSessionAndGroup(String sessionName, String groupName) {
		boolean flag = false;
		CuratorFramework client = threadLocalClient.getCuratorClient().getCuratorClient();
 		String path = znodeApi.makePath(Constants.EJOB_SERVER_NODE_PROJECT, Constants.PATH_SEPARATOR+sessionName, Constants.PATH_SEPARATOR+groupName);
    	...
    	ConcurrentHashMap<String, List<String>> projectSessionMap =  SlaveUtils.buildProjectSessionMap(client);
 		...
 		String node = znodeApi.makePath(Constants.ROOT, Constants.EJOB_RPC_NODE_ROOT, Constants.PATH_SEPARATOR+groupName);	  
									...
									//根据group,jobname添加新的iphost,iphost对应的rpc地址的值则根据job之前的值来替换.最后把新得到的rpcurl添加到节点上
  									if(rpcList !=null && rpcList.size() >0){
  										String rocUrl = rpcList.get(0);
  											...
  											String rpcNodePath = znodeApi.makePath(node, Constants.PATH_SEPARATOR+jobName, Constants.PATH_SEPARATOR+Constants.EJOB_PROVIDERS,Constants.PATH_SEPARATOR+ipHost);		
  											if(!znodeApi.checkExists(client, rpcNodePath)){
	...
	public static void main(String[] args) {
		 String str ="ejob://10.40.6.89:9501/test300?serverGroup=oms100test2&phpFilePath=/usr/local/php-test/TestService.php&className=TestService&methodName=test&version=1";
	...
	public boolean bindSessionAndGroup(String sessionName, String unaviableGroup ,String newGroup) {
		...
		//移除session与group的关系
		String path = znodeApi.makePath(Constants.EJOB_SERVER_NODE_PROJECT, Constants.PATH_SEPARATOR+sessionName, Constants.PATH_SEPARATOR+unaviableGroup);
	...
	private void removeBindGroupInfo(String jobGroup){
		...
		List<String> sessions = znodeApi.getChildren(client, Constants.EJOB_SERVER_NODE_SESSION);
		for(String session : sessions){
			String sessionGroupPath = ZKPaths.makePath(Constants.EJOB_SERVER_NODE_PROJECT, session, jobGroup);
	...
	public List<JobGroup> getAllJobGroupForPageList() {
		...
		CuratorFramework client = threadLocalClient.getCuratorClient().getCuratorClient();
        ZnodeApiCuratorImpl znode = new ZnodeApiCuratorImpl();
        List<String> groupList = znode.getChildren(client, Constants.ROOT+Constants.EJOB_RPC_NODE_ROOT);
        if(CollectionUtils.isNotEmpty(groupList)){
             for(String groupName : groupList){
            	 JobGroup jobGroup = new JobGroup(groupName);
            	 
            	 List<String> sessionList = znode.getChildren(client, Constants.EJOB_SERVER_NODE_PROJECT);
            	 for(String sessionName : sessionList){
            		 String publishSessionGroupPath = ZKPaths.makePath(Constants.EJOB_SERVER_NODE_PROJECT, sessionName, groupName);
