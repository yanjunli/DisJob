...
import com.hqyg.disjob.register.core.sharing.JobShardingStrategy;
import com.hqyg.disjob.register.core.sharing.JobShardingStrategyFactory;
import com.hqyg.disjob.register.core.sharing.JobShardingStrategyOption;
...
import com.hqyg.disjob.rpc.cluster.loadbalance.RoundRobinLoadBalance;
import com.hqyg.disjob.slaver.utils.SharingItem2StrategyOption;
...
public abstract class AbstractJobFactory implements Job{
	...
	protected void executeJobInternal(JobDataMap map){
        ...
        com.hqyg.disjob.register.domain.Job job = (com.hqyg.disjob.register.domain.Job) map.get(CoreConstants.JOB_CONFIG);
        ...
        if(isCanScheduler){
			...
			} catch (EjobFrameWorkException e) {
				LoggerUtil.error(e.getMessage());
			}
        }
	}

	private boolean checkCanScheduler(com.hqyg.disjob.register.domain.Job job) {
	...
	protected void processJob(String groupName, String jobName, String parameters,com.hqyg.disjob.register.domain.Job job, boolean isBroadcast) {
        ...
        if(CollectionUtils.isEmpty(urlLst)){
        	...
        	throw new EjobFrameWorkException("get rpc List<HURL> is null , job not executed");
        }
	...
	private void processRequest(com.hqyg.disjob.register.domain.Job job,List<HURL> urlLst, boolean isBroadcast) {
		if(isBroadcast){
			...
			if(hurl == null){
				throw new EjobFrameWorkException("no hurl for the job, groupName:" + job.getGroupName() + " jobName:" + job.getJobName() + " ,job not executed");
			}
			RegisterSpringWorkFactory.getJobExecutedThreadPoolService().submit(job, hurl,job.getParameters(), "");
		}
	}

	private void sharingProcessRequest(com.hqyg.disjob.register.domain.Job job,List<HURL> urlLst, boolean isBroadcast) {
