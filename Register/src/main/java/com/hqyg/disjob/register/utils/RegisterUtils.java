package com.hqyg.disjob.register.utils;
...
import com.hqyg.disjob.register.repository.watch.listener.SlaveIpNodeListener;
...
public class RegisterUtils {
	...
	public static String toServicePath(HURL url) {
		return toGroupPath(url) + EJobConstants.PATH_SEPARATOR + url.getServerName();
	}

	public static String toNodeTypePath(HURL url, ZkNodeType nodeType) {
		String type = "";
		if (nodeType == ZkNodeType.PROVIDER) {
			type = Constants.EJOB_PROVIDERS;
		} /*
			 * else if (nodeType == ZkNodeType.UNAVAILABLE_SERVER) { type =
			 * "unavailbleServer"; }
			 */ else if (nodeType == ZkNodeType.CONSUMER) {
			type = Constants.EJOB_CONSUMERS;
		} else {
			throw new EjobFrameWorkException(
					String.format("Failed to get nodeTypePath, url: %s type: %s", url, nodeType.toString()));
		}
		return toServicePath(url) + EJobConstants.PATH_SEPARATOR + type;
	}

	public static String toNodePath(HURL url, ZkNodeType nodeType) {
		return toNodeTypePath(url, nodeType) + EJobConstants.PATH_SEPARATOR + url.getServerPortStr();
	}

	public static String toGroupPath(HURL url) {
		return EJobConstants.ZOOKEEPER_REGISTRY_NAMESPACE + EJobConstants.PATH_SEPARATOR + url.getGroup();
	}
	...
	public static List<HURL> nodeChildsToUrls(CuratorFramework client, String parentPath, List<String> currentChilds) {
		...
		for (String node : currentChilds) {
			String nodePath = parentPath + EJobConstants.PATH_SEPARATOR + node;
			...
			String data = "";
			try {
				data = new String(bytes, EJobConstants.DEFAULT_CHARACTER);
