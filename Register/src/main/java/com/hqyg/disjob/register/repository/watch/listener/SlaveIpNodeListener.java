 ...
 * 
 *  Description:
 *  master监控/ejob/scheduler/slave/ip节点,发现slave挂掉后
			...
			if (array.length > 4) {
				...
				if (StringUtils.isNoneEmpty(tmp) && tmp.equalsIgnoreCase("status")) {
					...
					try {
						...
						LoggerUtil.error("SlaveIpNodeListener.childEvent distributeSlave and clearSlaveJobByTransaction got an exception ", e);
					}
				}
				...
				if (slaveIPs != null && !slaveIPs.isEmpty()) {
					for (String slaveIP : slaveIPs) {
						String slaveIPStatusPath = ZKPaths.makePath(slavePath, slaveIP, Constants.EJOB_SERVER_NODE_SLAVE_STATUS);
						if (nodeApi.checkExists(client, slaveIPStatusPath) && Constants.EJOB_SLAVE_STATUS.equals(nodeApi.getData(client, slaveIPStatusPath))) {
							available++;
				...
				if (available == 1) {
					notifySingleEjobAvailable(availableIps, unAvailableIps);
				} else {
					notifyPartEjobUnAvailable(availableIps, unAvailableIps);
				}
			}
		}
	}

	private void notifyPartEjobUnAvailable(Set<String> availableIps, Set<String> unAvailableIps) {
		CommonRMSMonitor.sendSystem(MonitorType.System.EJOB_EXCEPTION, "ejob part unavaiable! availableIps[" + availableIps + "] unAvailableIps[" + unAvailableIps + "]");
	}

	private void notifySingleEjobAvailable(Set<String> availableIps, Set<String> unAvailableIps) {
		CommonRMSMonitor.sendSystem(MonitorType.System.SERIOUS_EJOB_SIMGLE_AVAILABLE, "ejob only one avaiable! availableIps[" + availableIps + "] unAvailableIps[" + unAvailableIps + "]");
	}
}
