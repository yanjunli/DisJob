...
public class UserAuthNode {
	...
	public List<AuthInfo> getAuthInfos(){
		...
		switch (username) {
		case "admin":
			authInfolist.add(new EjobAuthInfo(znode.getByteData(client, AuthConstants.adminRootPath)));
			break;
		case "visitor":
			authInfolist.add(new EjobAuthInfo(znode.getByteData(client, AuthConstants.visitorRootPath)));
			break;
		default:
			...
			if(znode.checkExists(client, userAuthPath)){
				...
				for(String group : groups){
					...
					for(String authType : authTypes){
						...
						authInfolist.add(new EjobAuthInfo(znode.getByteData(client, authAccountPath)));
					}
				}
			}
			break;
