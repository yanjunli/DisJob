...
public class EjobServerServiceImpl implements EjobServerService {
	@Resource
	...
	public boolean distributeJob(CuratorFramework client) {
		...
		String slavePath = ZKPaths.makePath(Constants.ROOT, Constants.EJOB_SERVER_NODE_ROOT,
		...
		for (Map.Entry<String, List<String>> entry : groupJobMap.entrySet()) {
			...
			String slaveJobPath = ZKPaths.makePath(slavePath, Constants.PATH_SEPARATOR + ip,
					Constants.EJOB_SERVER_NODE_SLAVE_EXECUTION, Constants.PATH_SEPARATOR + entry.getKey(),
	...
	@Override
	public List<EjobServerInfo> getServerInfos(){
		List<EjobServerInfo> serverLst = new ArrayList<EjobServerInfo>();
       ...
       // String masterIp = znodeApi.getData(client, masterHostPath);
		...
		for (String slaveIp : slaveIpLst) {
			EjobServerInfo serverInfo = new EjobServerInfo(slaveIp);
			String ipPath = ZKPaths.makePath(slavePath, Constants.PATH_SEPARATOR + slaveIp);
			String hostNamePath = ZKPaths.makePath(ipPath, Constants.EJOB_SERVER_NODE_SLAVE_HOSTNAME);
			String statusPath = znodeApi.makePath(ipPath, Constants.EJOB_SERVER_NODE_SLAVE_STATUS);
	...
	@Override
	public Map<String, List<String>> getServerJob(EjobServerInfo server){
		Map<String, List<String>> groupJobMap = new HashMap<String, List<String>>();
		if(server == null){
			return groupJobMap;
		}
	...
	public HURL getHurlByJobAndIp(String group, String jobName, final String ip) {
		CuratorFramework client = threadLocalClient.getCuratorClient().getCuratorClient();
		String slavePath = ZKPaths.makePath(Constants.ROOT, Constants.EJOB_RPC_NODE_ROOT,  Constants.PATH_SEPARATOR+group,Constants.PATH_SEPARATOR+jobName,Constants.PATH_SEPARATOR+Constants.EJOB_PROVIDERS);
	...
	public List<String> getSessionsList() {
		...
		CuratorFramework client = threadLocalClient.getCuratorClient().getCuratorClient();
		String sessionRootPath = Constants.EJOB_SERVER_NODE_SESSION;
		List<String> sessionNames = znodeApi.getChildren(client, sessionRootPath);
		for(String session : sessionNames){
			String sessionPath = znodeApi.makePath(Constants.EJOB_SERVER_NODE_SESSION, session);
