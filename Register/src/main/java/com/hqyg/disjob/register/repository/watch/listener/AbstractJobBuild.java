 ...
 * 
 *  Description:
 *  监听/ejob/rpc和监听/ejob/rpc/group的时候共同的父类,
 *  功能：用于监听后创建节点
 * 
 *  Revision History
 *  Date,					Who,					What;
...
public abstract class AbstractJobBuild implements PathChildrenCacheListener{   
    public void buildJobByRPC(CuratorFramework client, String path) {
         ...
         String node = path;
         node = node.replace(Constants.EJOB_RPC_NODE_ROOT, Constants.APP_JOB_NODE_ROOT);
         ZnodeApiCuratorImpl znode = new ZnodeApiCuratorImpl();
         String [] temp =  node.split(Constants.PATH_SEPARATOR);
          //如果是类似/ejob/job/oms/server1结构则直接判断是否存在/ejob/job/oms/server1/config->Jobstatus,不存在则创建并设置值
         if(temp.length >=5){
    ...
    /**
     * 如果监听到ejob/rpc/group/servevr节点的新增则创建/ejob/job/group/job/config节点
     * @param client
     * @param node
     * @param znode
     * @param temp
	...
	private void buildJob(CuratorFramework client, String node,ZnodeApiCuratorImpl znode, String groupName,String jobName) {
		...
		String tempPath= znode.makePath(node, Constants.EJOB_CONFIG);
