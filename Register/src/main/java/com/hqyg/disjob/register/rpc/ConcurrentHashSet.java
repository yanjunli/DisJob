...
public class ConcurrentHashSet<E> extends AbstractSet<E> implements Set<E>, java.io.Serializable {
    ...
    public void clear() {
	 ...
	 public static void main(String[] args) {
		ConcurrentHashSet<EjobServerInfo> set = new ConcurrentHashSet<EjobServerInfo>();
		set.add(new EjobServerInfo("10.38.6.99"));

		set.add(new EjobServerInfo("192.168.1.0"));
		set.add(new EjobServerInfo("192.168.1.1"));
		set.add(new EjobServerInfo("192.168.1.2"));
		set.add(new EjobServerInfo("192.168.1.3"));
		set.add(new EjobServerInfo("10.37.6.56"));
		set.add(new EjobServerInfo("10.38.6.57"));
		set.add(new EjobServerInfo("10.38.6.59"));
		set.add(new EjobServerInfo("10.38.6.54"));



		 TreeSet<EjobServerInfo> treeSet = new TreeSet<EjobServerInfo>(new Comparator<EjobServerInfo>() {

			@Override
			public int compare(EjobServerInfo o1, EjobServerInfo o2) {
		 ...
		 treeSet.addAll(set);
		// TreeSet<EjobServerInfo> treeSet = new TreeSet<EjobServerInfo>(set);
		Iterator<EjobServerInfo>  item =  treeSet.iterator();
		while(item.hasNext()){
			EjobServerInfo info = item.next();
			//System.out.println(info.getIp()+"====="+info.getIp().hashCode());
					
		}

		
		Iterator<EjobServerInfo>  it =  set.iterator();
