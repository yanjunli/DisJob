package com.hqyg.disjob.slaver.utils;
...
import com.hqyg.disjob.register.domain.SlaveNode;
import com.hqyg.disjob.register.job.JobOperationServiceImpl;
...
public class SlaveUtils
{
   ...
   static ZnodeApiCuratorImpl nodeApi = new ZnodeApiCuratorImpl();
     ...
     * @return
     */
    public static  ConcurrentHashSet<EjobServerInfo> getAvailableAlaveIps(CuratorFramework client){
        String slaveNode = Constants.ROOT+Constants.EJOB_SERVER_NODE_ROOT+Constants.EJOB_SERVER_NODE_SLAVE;
        ZnodeApiCuratorImpl znode = new ZnodeApiCuratorImpl();
        List<String> currentChilds = znode.getChildren(client, slaveNode);
        ConcurrentHashSet<EjobServerInfo> result = new ConcurrentHashSet<EjobServerInfo>();
        if(CollectionUtils.isNotEmpty(currentChilds)){
            for(String slaveIp : currentChilds){
                String dataNode = slaveNode + Constants.PATH_SEPARATOR + slaveIp+ Constants.EJOB_SERVER_NODE_SLAVE_STATUS;
                if(znode.checkExists(client, dataNode)){
                    //暂时不对status的值做验证,因为目前服务不可用自动status节点就移除啦
                    //TODO这里可以做IP权重设置,读取配置文件设置ip对应权重
                    EjobServerInfo slaveServer = new EjobServerInfo(slaveIp, Constants.EJOB_SLAVE_DEFAULT_WEIGHT);
...
//       return nodeApi.addUpdateToTransaction(transaction, jobNode, json);
    ...
    public static ConcurrentHashMap<String,String>  getGroupAndJobMapByIp(CuratorFramework client,String ip){
        ZnodeApiCuratorImpl nodeApi = new ZnodeApiCuratorImpl(); 
        String ipNode = nodeApi.makePath(Constants.ROOT, Constants.EJOB_SERVER_NODE_ROOT, Constants.EJOB_SERVER_NODE_SLAVE,Constants.PATH_SEPARATOR+ip,Constants.EJOB_SERVER_NODE_SLAVE_EXECUTION);
     ...
     * @param client
     */
    public static ConcurrentHashSet<EjobServerInfo> refreshSlaveIp(CuratorFramework client){
        //刷新IP
        ConcurrentHashSet<EjobServerInfo> result = new ConcurrentHashSet<EjobServerInfo>();
        ConcurrentHashSet<EjobServerInfo> list = SlaveUtils.getAvailableAlaveIps(client);
        if(CollectionUtils.isNotEmpty(list)){
            for(EjobServerInfo info : list){
            ...
            for(EjobServerInfo filter:ZKJobCache.ipList){
    ...
    public static void clearSlaveJob(String ip,CuratorFramework client){
        ...
        ZnodeApiCuratorImpl nodeApi =  new ZnodeApiCuratorImpl();
        String executionNode = nodeApi.makePath(Constants.ROOT, Constants.EJOB_SERVER_NODE_ROOT,Constants.EJOB_SERVER_NODE_SLAVE,Constants.PATH_SEPARATOR+ip,Constants.EJOB_SERVER_NODE_SLAVE_EXECUTION);
	...
	public static String distributeSlave(CuratorFramework client, String group,String jobName,CuratorTransaction transaction) throws Exception {
        EjobServerInfo slaver =  WeightedRoundRobinScheduling.GetBestSlaveServer(client);
        if(slaver == null){
            LoggerUtil.warn("WeightedRoundRobinScheduling.GetBestSlaveServer get the EjobServerInfo is null, so return itself, but this phenomenon is not common. because at least it should return itself ip info ");
            //重试一次
            SlaveUtils.refreshSlaveIp(client);
            EjobServerInfo trySlaver =  WeightedRoundRobinScheduling.GetBestSlaveServer(client);
            if(trySlaver != null){
	...
	public static synchronized void updateSlaveExecution(CuratorFramework client,String group, String jobName, CuratorTransaction transaction,String ip) {
		ZnodeApiCuratorImpl nodeApi = new ZnodeApiCuratorImpl();
		String groupNode = nodeApi.makePath(Constants.ROOT,Constants.EJOB_SERVER_NODE_ROOT,Constants.EJOB_SERVER_NODE_SLAVE,Constants.PATH_SEPARATOR + ip,Constants.EJOB_SERVER_NODE_SLAVE_EXECUTION,Constants.PATH_SEPARATOR + group);
    ...
    public static List<String> getAllOfflineIpsToDistribute(CuratorFramework client){
     	 String path = nodeApi.makePath(Constants.ROOT,Constants.EJOB_SERVER_NODE_ROOT,Constants.EJOB_SERVER_NODE_SLAVE);
    	 List<String> ips = nodeApi.getChildren(client, path);
    	 List<String> needDistributeIps = new ArrayList<String>();
    	 if(CollectionUtils.isNotEmpty(ips)){
    		 for(String ip : ips){
    			String statusNode = nodeApi.makePath(path,  Constants.PATH_SEPARATOR+ip, Constants.EJOB_SERVER_NODE_SLAVE_STATUS);
     			 //如果该节点不存在,则查看
    			 if(!nodeApi.checkExists(client, statusNode)){
    				 String executionNode = nodeApi.makePath(path,  Constants.PATH_SEPARATOR+ip, Constants.EJOB_SERVER_NODE_SLAVE_EXECUTION);
    ...
    public static void clearAllClustersJobs(CuratorFramework client,CuratorTransaction transaction) throws Exception{
     	String slaveNode = Constants.ROOT+Constants.EJOB_SERVER_NODE_ROOT+ Constants.EJOB_SERVER_NODE_SLAVE;//"/ejob/scheduler/slave";
    ...
    public static void  distributeAllJobs(CuratorFramework client) throws Exception{
   		Shard<SlaveNode> shard = com.hqyg.disjob.slaver.utils.Shard.initSlaveIpInShard(client);
   		ZnodeApiCuratorImpl znode = new ZnodeApiCuratorImpl();
    ...
    public static ConcurrentHashMap<String, List<String>> buildSessionHostMap(CuratorFramework client) {
    	ZnodeApiCuratorImpl znode = new ZnodeApiCuratorImpl();
    	ConcurrentHashMap<String, List<String>> sessionHostMap = com.hqyg.disjob.register.cache.ZKJobCache.sessionHostMap; 
 		String sessionNode = com.hqyg.disjob.common.Constants.EJOB_SERVER_NODE_SESSION;
		//得到绑定会话源与iphost关系
    ...
    public static ConcurrentHashMap<String, List<String>> buildProjectSessionMap(CuratorFramework client) {
      	String projectNode = com.hqyg.disjob.common.Constants.EJOB_SERVER_NODE_PROJECT;
    ...
    public static String buildRpcUrl(Job job){
    	...
    	sb.append("ejob://").append(job.getHost());
    ...
    public static List<String> getProviderUrlNodeByGroupAndHost(CuratorFramework client,String group,String ipPort){
		ZnodeApiCuratorImpl znode = new ZnodeApiCuratorImpl();
		String node = znode.makePath(Constants.ROOT, Constants.EJOB_RPC_NODE_ROOT, Constants.PATH_SEPARATOR+group);	  
		List<String> jobNameList = znode.getChildren(client, node);
		List<String> result = new ArrayList<String>();
		if(CollectionUtils.isNotEmpty(jobNameList)){
			for(String jobName : jobNameList){
				
				String deletePath = znode.makePath(node, Constants.PATH_SEPARATOR+jobName, Constants.PATH_SEPARATOR+Constants.EJOB_PROVIDERS,Constants.PATH_SEPARATOR+ipPort);
